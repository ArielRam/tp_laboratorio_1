 ...
 *
 */
int print_and_get_menu_option_movies()
{
    int opcion=0;
 ...
 *
 * \param peliculas array de estructura eMovie
 * \param len int tamaño del array
 * \return void
 *
 */
void addMovie(eMovie peliculas[], int len)
{
    int freeP=0;
    int findMov=0;
    int durAux = 0;
    ...
    freeP=findFreeMoviePlace(peliculas, len);

        while(aux == 0)
        {
        	aux = getStrings("Ingrese Titulo: ",peliculas[freeP].title);
        
            if (aux == 0)
	            printf("\nTitulo invalido! \n");
        }            

        durAux = getInt("Ingrese duracion en minutos: ");
        ...
        while(aux == 0)
        {
        	aux = getStrings("Ingrese Categoria: ",peliculas[freeP].category);
        ...
        while(auxDesc == 0)
        {
        	auxDesc = getStrings("Ingrese Descripcion: ",peliculas[freeP].description);
        
            if (auxDesc == 0)
	            printf("\nDescripcion invalida! \n");
        }

        auxRat = getInt("Ingrese rating: ");
        ...
        while(auxRat == 0)
        {
            ...
            if (auxRat == 0)
	            printf("\rating invalido! \n");
        }
		
		peliculas[freeP].rating = auxRat;
		peliculas[freeP].duration = durAux;


        peliculas[freeP].status=1;
        printf("\nLa carga ha sido exitosa!\n\n");

}

/** \brief dar de baja una pelicula
 *
 * \param peliculas array de estructura eMovie
 * \param len int tamaño del array
 * \return void
 *
 */
void deleteMovie(eMovie peliculas[], int len)
{
    float idAux=0;
    int idx=0;
    int confirm=0;
	...
	if (idAux == 0){
        printf("\nEl ingresado Id es invalido");
        return;
	}

    idx = findById(peliculas, len, idAux);

    if(idx == -1)
       printf("\nEl ID ingresado no existe");
    else
    {
        printf("\nTITULO  CATEGORIA  DURACION   RATING DESCRIPCION");
        printf("\n\n");
        printf("%s - %s - %d - %d - %s", peliculas[idx].title, peliculas[idx].category, peliculas[idx].duration, peliculas[idx].rating, peliculas[idx].description);
        
        printf("\n\n");

        printf("Esta seguro que desea eliminar esta pelicula?");
        printf("\n\n");
        printf("1. Si\n");
        printf("2. No\n");
        printf("\nOpcion: ");
 ...
 *
 */
void editMovie(eMovie peliculas[])
{
	// implementar
}
 ...
 *
 */
void createHmtlPage(eMovie peliculas[], int len)
{
	// implementar
    char name;
    int i;
    ...
    for(i = 0; i < len; i++)
    {
		fprintf(fp,"%s","<article class='col-md-4 article-intro'>");
		fprintf(fp,"%s","<h3>");
		fprintf(fp,"%s","<a href='#'>");
		fprintf(fp,"%s",peliculas[i].title);
		// fprintf(fp,"%s","</a></h3>");
		// fprintf(fp,"%s","<ul>");
		// fprintf(fp,"%s","<li>");
		// fprintf(fp,"%s","</li><li>Puntaje:");
		// fprintf(fp,"%s",peliculas[i].rating);
		// fprintf(fp,"%s","</li><li>Duración:116</li>");					
		// fprintf(fp,"%s","</ul>");
		// fprintf(fp,"%s","<p>");
		// fprintf(fp,"%s",peliculas[i].description);
		// fprintf(fp,"%s","</p></article>");
	}
...
/** \brief Inicializa posiciones del estado en 0
 *
 * \param personas array de eestructura eMovie
 * \param len tamaño del array 
 * \return void
 *
 */
void initializeMovies(eMovie peliculas[], int len)
{
    int i;

    for(i=0; i<len; i++){
        peliculas[i].status=0;
        peliculas[i].id=i+1;
    }
}
 ...
 *
 */
int findFreeMoviePlace(eMovie peliculas[], int len)
{
    int idx=-1;
    int i;
...
/** \brief Busca DNI
 *
 * \param personas array de estructura peliculas
 * \param len int tamaño del array
 * \param id int
 * \return posicion del dni or -1 int
 *
 */	
int findById(eMovie peliculas[], int len, int id)
{
    int idx=-1;
    int i;
